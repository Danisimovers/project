...
public class CatchJoinedDao implements CatchJoined {

    private static final Logger logger = LogManager.getLogger(CatchJoinedDao.class);
    ...
    public List<Catch> getAll() {
        ...
        try (Session session = HibernateUtil.getSessionFactory().openSession()) {
            List<Catch> catchEntities = session.createQuery("FROM CatchJoined", Catch.class).list();
